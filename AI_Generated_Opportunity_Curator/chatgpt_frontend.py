...
if st.session_state["user_mode"] == "Student":
    ...
    with tab3:
        st.title("Network Curator Chatbot (Llama2 Model)")
        ...
        if st.button("Send", key="network_send") and network_input:
            ...
            try:
                # Create system prompt with combined user profile and resume context
                system_prompt = f"System: You are a network curator. Use this user profile and resume information to provide personalized networking recommendations: {st.session_state['user_profile_text']}\n\nUser: {network_input}"
